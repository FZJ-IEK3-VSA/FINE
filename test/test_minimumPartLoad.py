...
def test_minimumPartLoad(solver):
    ...
    esM = fn.EnergySystemModel(locations=locations, commodities=commodities, numberOfTimeSteps=5,
    ...
    esM.add(fn.Conversion(esM=esM, name='unrestricted', physicalUnit=r'GW$_{el}$',
    ...
    esM.add(fn.Conversion(esM=esM, name='restricted', physicalUnit=r'GW$_{el}$',
                          commodityConversionFactors={'electricity':1, 'methane':-1/0.625},
                          capacityFix = data_cap, partLoadMin = 0.4, bigM= 10000,
                          investPerCapacity=0.5, opexPerCapacity=0.015, interestRate=0.08,
                          economicLifetime=33))
    ...
    print(esM.componentModelingDict['ConversionModel'].operationVariablesOptimum.xs('restricted'))
    print('unrestricted dispatch:\n')
    print(esM.componentModelingDict['ConversionModel'].operationVariablesOptimum.xs('unrestricted'))
    # test if here solved fits with original results
    
     # test if here solved fits with original results
    testresults = esM.componentModelingDict["ConversionModel"].operationVariablesOptimum.xs('restricted')
