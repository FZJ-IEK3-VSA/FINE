...
def test_aggregate_time_series_mean_and_sum(
    ...
    assert (
        time_series_aggregated.loc["source_comp", "T0", "01_reg_02_reg"].values
        == expected
    )

    ## for invalid component, sink_comp
    assert np.isnan(
        time_series_aggregated.loc["sink_comp", "T0", "01_reg_02_reg"].values
    )
...
def test_aggregate_time_series_weighted_mean(
    ...
    weight_xr = xr.DataArray(
        weight, coords=[component_list, space_list], dims=["component", "space"]
    )
    ...
    assert (
        time_series_aggregated.loc["source_comp", "T0", "03_reg_04_reg"].values
        == expected_grp2
    )
...
@pytest.mark.parametrize(
    "mode, expected_for_valid_component",
    [
...
def test_aggregate_connections(
    xr_and_dict_for_basic_representation, mode, expected_for_valid_component
):
    ...
    assert output_for_invalid_component[0][0] == 0 and np.isnan(
        output_for_invalid_component[0][1]
    )
    ...
    (
        {
            "operationRateMax": (
                ...
                xr.DataArray(
                    ...
                    coords=[
                        ["source_comp", "sink_comp", "transmission_comp"],
...
def test_aggregate_based_on_sub_to_sup_region_id_dict(
    ...
    output_xarray = spr.aggregate_based_on_sub_to_sup_region_id_dict(
        test_xr,
        sub_to_sup_region_id_dict,
    ...
    assert (
        ...
        .loc["source_comp", "T0", "01_reg_02_reg"]
        .values
        == expected_ts_operationRateMax
    )
    assert (
        output_xarray["ts_operationRateFix"]
        .loc["sink_comp", "T0", "01_reg_02_reg"]
        .values
        == expected_ts_operationRateFix
    )
    ...
    ## 1d variable
    assert (
        output_xarray["1d_capacityMax"].loc["source_comp", "01_reg_02_reg"].values
        == expected_1d_capacityMax
    )
    assert (
        output_xarray["1d_capacityFix"].loc["sink_comp", "01_reg_02_reg"].values
        == expected_1d_capacityFix
    )

    ## 2d variable
    output_2d_capacityMax = (
        output_xarray["2d_capacityMax"].loc["transmission_comp"].values
    )
    assert np.array_equal(output_2d_capacityMax, expected_2d_capacityMax)

    output_2d_locationalEligibility = (
        output_xarray["2d_locationalEligibility"].loc["transmission_comp"].values
    )
    assert np.array_equal(
        output_2d_locationalEligibility, expected_2d_locationalEligibility
    )
