...
def test_exceededLifetime():
    ...
    costs = pd.DataFrame([np.array([ 0.05, 0., 0.1, 0.051,])],
                            index = ['OneLocation']).T
    revenues = pd.DataFrame([np.array([ 0., 0.01, 0., 0.,])],
                            index = ['OneLocation']).T
    maxpurchase = pd.DataFrame([np.array([1e6, 1e6, 1e6, 1e6,])],
    ...
    ### Industry site
    demand = pd.DataFrame([np.array([6e3, 6e3, 6e3, 6e3,])],
                    index = ['OneLocation']).T * hoursPerTimeStep
