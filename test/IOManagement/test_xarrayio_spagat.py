...
def test_generateIterationDicts(minimal_test_esM):
    ...
    (
        output_df_dict,
        output_series_dict,
        output_constants_dict,
    ) = utilsIO.generateIterationDicts(component_dict)

    # check output_df_dict
    assert output_df_dict == {
        "operationRateMax": [("Source", "Electricity market")],
    ...
    assert output_series_dict.get("commodityRevenue") == [
        ("Source", "Electricity market"),
        ("Sink", "Industry site"),
    ]

    assert output_series_dict.get("opexPerChargeOperation") == [
        ("Storage", "Pressure tank")
    ]

    # check output_constants_dict
    assert (
        len(output_constants_dict.get("name")) == 5
    )  # 5 components present in the esM instance
    assert output_constants_dict.get("physicalUnit") == [
        ("Conversion", "Electrolyzers")
    ]
    assert output_constants_dict.get("commodityConversionFactors.electricity") == [
        ("Conversion", "Electrolyzers")
    ]

    # In some cases, when a varaible is not present, it is None and is captured by output_constants_dict
    assert set(output_df_dict.get("operationRateFix")).isdisjoint(
        output_constants_dict.get("operationRateFix")
    )
...
def test_convertEsmInstanceToXarrayDataset(multi_node_test_esM_init):
    ...
    ## commodities
    assert (
        output_xarray.attrs.get("commodities") == multi_node_test_esM_init.commodities
    )
    ...
    assert (output_2d[0][0] == 0.0) and (
        output_2d[3][6] == expected_2d["cluster_3_cluster_6"]
    )
    ...
    expected_0d = multi_node_test_esM_init.getComponentAttribute(
        "Electroylzers", "commodityConversionFactors"
    ).get("electricity")
    output_0d = (
...
def test_convertXarrayDatasetToEsmInstance(multi_node_test_esM_init):
    ...
    init_esm_ts = multi_node_test_esM_init.getComponentAttribute(
    ...
    ## a 2d variable
    init_esm_2d = multi_node_test_esM_init.getComponentAttribute(
        "Pipelines (biogas)", "locationalEligibility"
    )
    ...
    output_esm_2d = output_esM.getComponentAttribute(
        "Pipelines (biogas)", "locationalEligibility"
    )
    ...
    ## a 1d variable
    init_esm_1d = multi_node_test_esM_init.getComponentAttribute(
    ...
    ## a constant
    init_esm_0d = multi_node_test_esM_init.getComponentAttribute(
    ...
    ## a constant, bool
    init_esm_0d_bool = multi_node_test_esM_init.getComponentAttribute(
        "New CCGT plants (hydrogen)", "hasCapacityVariable"
    )
    test_xarray_0d_bool = (
...
def test_convertEsmInstanceToXarrayDataset_singlenode(single_node_test_esM):
    ...
    ## time series
    expected_ts = single_node_test_esM.getComponentAttribute(
    ...
    ## constant
    expected_0d = single_node_test_esM.getComponentAttribute(
        "Electrolyzers", "commodityConversionFactors"
    ).get("electricity")
    output_0d = (
        output_xarray["0d_commodityConversionFactors.electricity"]
    ...
    ## constant bool
    expected_0d_bool = single_node_test_esM.getComponentAttribute(
...
def test_convertXarrayDatasetToEsmInstance_singlenode(single_node_test_esM):
    ...
    ## a time series variable
    init_esm_ts = single_node_test_esM.getComponentAttribute(
    ...
    ## a 1d variable
    init_esm_1d = single_node_test_esM.getComponentAttribute(
    ...
    ## a constant
    init_esm_0d = single_node_test_esM.getComponentAttribute(
    ...
    ## a constant, bool
    init_esm_0d_bool = single_node_test_esM.getComponentAttribute(
        "Pressure tank", "hasCapacityVariable"
    )
    test_xarray_0d_bool = (
    ...
    output_esM.aggregateTemporally(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pd.DataFrame(
            np.array([[1, 2, 3, 4, 5, 6, 7, 8], [5, 6, 7, 8, 9, 0, 1, 2]]),
            columns=[
                "cluster_0",
                "cluster_1",
                "cluster_2",
...
def test_savingAndReadingNetcdfFiles(balanceLimit, multi_node_test_esM_init):
    ...
    xrIO.convertEsmInstanceToXarrayDataset(
        multi_node_test_esM_init, save=True, file_name=file
    )
    ...
    assert output_esM.getComponentAttribute(
        "Biogas purchase", "commodity"
    ) == multi_node_test_esM_init.getComponentAttribute("Biogas purchase", "commodity")

    assert output_esM.getComponentAttribute(
        "New CCGT plants (biogas)", "commodityConversionFactors"
    ) == multi_node_test_esM_init.getComponentAttribute(
        "New CCGT plants (biogas)", "commodityConversionFactors"
    )

    assert np.array_equal(
        output_esM.getComponentAttribute("PV", "capacityMax"),
        multi_node_test_esM_init.getComponentAttribute("PV", "capacityMax"),
    )

    assert np.array_equal(
        output_esM.getComponentAttribute("Li-ion batteries", "investPerCapacity"),
        multi_node_test_esM_init.getComponentAttribute(
            "Li-ion batteries", "investPerCapacity"
        ),
    )
