...
def stochasticESM(singleYear=False, sameParameters=False,transmissionCase=False):
    ...
    yearlyCostsElec=pd.DataFrame(
        columns=locIndex, 
        index=[x for x in range(0,numberOfTimeSteps)],
        data=1)
    if singleYear:
    ...
    for col in yearlyMaxPurchaseElec: # so it works for one or several locations
        yearlyMaxPurchaseElec[col]=[0.5e3,0.5e3,4e3,4e3,]
    if singleYear:
    ...
    yearlyPVoperationRateMax=pd.DataFrame(
        columns=locIndex, 
        index=[x for x in range(0,numberOfTimeSteps)])
    for col in yearlyPVoperationRateMax.columns: # so it works for one or several locations
    ...
    if not transmissionCase:
        yearlyDemand = pd.DataFrame(
            columns=locIndex, 
            data=[2e3, 1e3, 1e3, 1e3])
        if singleYear:
            demand = yearlyDemand
        else:
            demand = {}
...
def test_stochasticBasic():
    ...
    assert cms_vars[("PerfectLand","PV",0)] == singleYear_cms_vars[("PerfectLand","PV",0)]
    assert cap_var[("PerfectLand","PV",0)] == singleYear_cap_vars[("PerfectLand","PV",0)]
...
def test_stochasticTimeSeries_withTransmission():
    ...
    # test demand
    assert list(
        esM.componentModelingDict["SourceSinkModel"].operationVariablesOptimum[0].xs("EDemand").loc["PerfectLand"].values
    ) == [2000, 1000, 1000, 1000]
    assert list(
        esM.componentModelingDict["SourceSinkModel"].operationVariablesOptimum[1].xs("EDemand").loc["PerfectLand2"].values
    ) == [0, 2000, 2000, 2000]
    
    # test objective value
    np.testing.assert_almost_equal(
        esM.pyM.Obj(), 19003.565679567248
    )
