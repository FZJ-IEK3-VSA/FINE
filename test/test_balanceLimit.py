...
def test_balanceLimitConstraint():
    ...
    esM.add(fn.Transmission(esM=esM, name='AC cables', commodity='electricity',
                            ...
                            distances=distances, losses=losses, balanceLimitID="el"))
    ...
    esM.add(fn.Transmission(esM=esM, name='Heat pipes', commodity='heat',
                            hasCapacityVariable=True, capacityFix=capacityFix,
                            distances=distances, losses=losses, balanceLimitID="heat"))
    ...
    for i, loc in enumerate(esM.locations):
        ...
        assert el_purchase + exch_in - exch_out - tolerance < balanceLimit.loc["el", loc]
        assert heat_purchase + exch_in_h - exch_out_h - tolerance < balanceLimit.loc["heat", loc]
...
def test_electricitySourceDriver():
    ...
    for i, loc in enumerate(esM.locations):
        ...
        assert operation_wind + operation_pv + tolerance > balanceLimit.loc["Renewables", loc]
...
def test_hydrogenSinkDriver():
    ...
    esM.add(fn.Sink(esM=esM, name="Hydrogen Annual Production", balanceLimitID="hydrogenDriver",
...
def test_CO2Limit():
    ...
    esM.add(fn.Sink(esM=esM, name='CO2 to environment', commodity='CO2',
                    hasCapacityVariable=False, balanceLimitID="CO2 limit"))
    ## Wind turbines
