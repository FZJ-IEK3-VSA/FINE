...
def test_minimumDownTime():
    ...
    esM.add(fn.ConversionFancy(esM=esM, name='unrestricted', physicalUnit=r'GW$_{el}$',
    ...
    esM.add(fn.ConversionFancy(esM=esM, name='restricted', physicalUnit=r'GW$_{el}$',
                          commodityConversionFactors={'electricity':1, 'methane':-1/0.625},
                          capacityFix = data_cap, partLoadMin = 0.4, bigM= 100, downTimeMin=3,
                          investPerCapacity=0.5, opexPerCapacity=0.021, opexPerOperation =1, interestRate=0.08,
                          economicLifetime=33))

    data_demand = {'example_region1': [5,5,2,2,4,5,5,2,2,4,5,5,2,2,4,5,5,2,2,4], 'example_region2': [5,5,2,2,4,5,5,2,2,4,5,5,2,2,4,5,5,2,2,4]}
    ...
    print(esM.componentModelingDict['ConversionFancyModel'].operationVariablesOptimum.xs('restricted'))
    print('unrestricted dispatch:\n')
    print(esM.componentModelingDict['ConversionFancyModel'].operationVariablesOptimum.xs('unrestricted'))
#    print(esM.componentModelingDict['ConversionFancyModel'].operationVariablesOptimum.xs('unrestricted'))
    # test if here solved fits with original results
    
     # test if here solved fits with original results
    testresults = esM.componentModelingDict["ConversionFancyModel"].operationVariablesOptimum.xs('restricted')
    np.testing.assert_array_almost_equal(testresults.values[0], results,decimal=2)
#   

if __name__ == "__main__":
    test_minimumDownTime()