...
# PERCENTILE PLOT ----------------------------------------------------
def plot_percentile_plot(
    original_capfac, RESULTS_PATH, data="Wind", clusters_list=["mean", 2, 5, 10]
):
    ...
    fig, ax = plt.subplots(
        nrows=len(clusters_list) + 1, ncols=1, sharey=True, sharex=True, figsize=(19, 8)
    )
    ...
    for cluster in clusters_list:
        ...
        if cluster == "mean":
            mean_capfac = xr.open_dataarray(
                RESULTS_PATH / "Mean" / f"{data}_mean_capfac.nc4"
            )
            ax[ax_no].plot(range(8760), mean_capfac, color="black")
            ax[ax_no].set_title("Mean of time series")
        else:
            # clustered data
            clustered_capfac = xr.open_dataarray(
                RESULTS_PATH / "Hier_Ward" / f"{data}_{cluster}cl_capfac.nc4"
            )
