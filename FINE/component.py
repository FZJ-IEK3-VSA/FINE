...
class Component(metaclass=ABCMeta):
    ...
    def __init__(self, esM, name, dimension,
        ...
        **Default arguments:**
        ...
        self._modelingClass = ComponentModel
        ...
        self._investPerCapacity = utils.checkAndSetCostParameter(esM, name, investPerCapacity, dimension, elig)
        self._investIfBuilt = utils.checkAndSetCostParameter(esM, name, investIfBuilt, dimension, elig)
        self._opexPerCapacity = utils.checkAndSetCostParameter(esM, name, opexPerCapacity, dimension, elig)
        self._opexIfBuilt = utils.checkAndSetCostParameter(esM, name, opexIfBuilt, dimension, elig)
        self._interestRate = utils.checkAndSetCostParameter(esM, name, interestRate, dimension, elig)
        self._economicLifetime = utils.checkAndSetCostParameter(esM, name, economicLifetime, dimension, elig)
        self._CCF = utils.getCapitalChargeFactor(self._interestRate, self._economicLifetime)

        # TODO adapt to series
        ...
        utils.checkLocationSpecficDesignInputParams(esM, hasCapacityVariable, hasIsBuiltBinaryVariable,
                                                    ...
                                                    dimension=dimension)
        self._locationalEligibility = locationalEligibility
    ...
    def addToEnergySystemModel(self, esM):
        ...
        if self._name in esM._componentNames:
            if esM._componentNames[self._name] == self._modelingClass.__name__:
            ...
            esM._componentNames.update({self._name: self._modelingClass.__name__})
        mdl = self._modelingClass.__name__
        if mdl not in esM._componentModelingDict:
            esM._componentModelingDict.update({mdl: self._modelingClass()})
    ...
    def prepareTSAInput(self, rateFix, rateMax, rateName, rateWeight, weightDict, data):
        ...
        if data_ is not None:
            data_ = data_.copy()
            uniqueIdentifiers = [self._name + rateName + loc for loc in data_.columns]
            data_.rename(columns={loc: self._name + rateName + loc for loc in data_.columns}, inplace=True)
    ...
    def getTSAOutput(self, rate, rateName, data):
        if rate is not None:
            uniqueIdentifiers = [self._name + rateName + loc for loc in rate.columns]
            data_ = data[uniqueIdentifiers].copy()
            data_.rename(columns={self._name + rateName + loc: loc for loc in rate.columns}, inplace=True)
            return data_
        else:
            return None
...
class ComponentModel(metaclass=ABCMeta):
    ...
    def initDesignVarSet(self, pyM):
        ...
        def initDesignVarSet(pyM):
            return ((loc, compName) for compName, comp in compDict.items()
                    for loc in comp._locationalEligibility.index
    ...
    def declareOperationModeSets(self, pyM, constrSetName, rateMax, rateFix):
        ...
        def initOpConstrSet1(pyM):
            return ((loc, compName) for loc, compName in varSet if compDict[compName]._hasCapacityVariable
        ...
        def initOpConstrSet4(pyM):
            return ((loc, compName) for loc, compName in varSet if not compDict[compName]._hasCapacityVariable
                    and getattr(compDict[compName], rateFix) is not None)
    ...
    def declareCapacityVars(self, pyM):
        ...
        def capBounds(pyM, loc, compName):
            ...
            return (comp._capacityMin[loc] if (comp._capacityMin is not None and not comp._hasIsBuiltBinaryVariable)
                    ...
                    comp._capacityMax[loc] if comp._capacityMax is not None else None)
    ...
    def capToNbReal(self, pyM):
        ...
        def capToNbReal(pyM, loc, compName):
            return capVar[loc, compName] == nbRealVar[loc, compName] * compDict[compName]._capacityPerPlantUnit
    ...
    def capToNbInt(self, pyM):
        ...
        def capToNbInt(pyM, loc, compName):
            return capVar[loc, compName] == nbIntVar[loc, compName] * compDict[compName]._capacityPerPlantUnit
    ...
    def bigM(self, pyM):
        ...
        def bigM(pyM, loc, compName):
            return capVar[loc, compName] <= designBinVar[loc, compName] * compDict[compName]._bigM
    ...
    def capacityMinDec(self, pyM):
        ...
        compDict, abbrvName, dim = self._componentsDict, self._abbrvName, self._dimension
        capVar, designBinVar = getattr(pyM, 'cap_' + abbrvName), getattr(pyM, 'designBin_' + abbrvName)
        designBinVarSet = getattr(pyM, 'designDecisionVarSet_' + abbrvName)

        def capacityMinDec(pyM, loc, compName):
            return (capVar[loc, compName] >= compDict[compName]._capacityMin[loc] * designBinVar[loc, compName]
                    if compDict[compName]._capacityMin is not None else pyomo.Constraint.Skip)
    ...
    def capacityFix(self, pyM):
        ...
        def capacityFix(pyM, loc, compName):
            return (capVar[loc, compName] == compDict[compName]._capacityFix[loc]
    ...
    def designBinFix(self, pyM):
        ...
        def designBinFix(pyM, loc, compName):
            return (designBinVar[loc, compName] == compDict[compName]._isBuiltFix[loc]
    ...
    def operationMode1(self, pyM, esM, constrName, constrSetName, opVarName, factorName=None, isStateOfCharge=False):
        ...
        factor1 = 1 if isStateOfCharge else esM._hoursPerTimeStep
    ...
    def operationMode2(self, pyM, esM, constrName, constrSetName, opVarName, isStateOfCharge=False):
        ...
        factor = 1 if isStateOfCharge else esM._hoursPerTimeStep
        ...
        def op2(pyM, loc, compName, p, t):
            return opVar[loc, compName, p, t] == capVar[loc, compName] * \
                   compDict[compName]._operationRateFix[loc][p, t] * factor
    ...
    def operationMode3(self, pyM, esM, constrName, constrSetName, opVarName, isStateOfCharge=False):
        ...
        def op3(pyM, loc, compName, p, t):
            return opVar[loc, compName, p, t] <= capVar[loc, compName] * \
                   compDict[compName]._operationRateMax[loc][p, t] * factor
    ...
    def operationMode4(self, pyM, esM, constrName, constrSetName, opVarName):
        ...
        def op4(pyM, loc, compName, p, t):
            return opVar[loc, compName, p, t] == compDict[compName]._operationRateFix[loc][p, t]
    ...
    def operationMode5(self, pyM, esM, constrName, constrSetName, opVarName):
        ...
        def op5(pyM, loc, compName, p, t):
            return opVar[loc, compName, p, t] <= compDict[compName]._operationRateMax[loc][p, t]
    ...
    def getSharedPotentialContribution(self, pyM, key, loc):
        ...
        return sum(capVar[loc, compName] / compDict[compName]._capacityMax[loc] for compName in compDict
                   if compDict[compName]._sharedPotentialID == key and (loc, compName) in capVarSet)

    def getLocEconomicsTD(self, pyM, esM, factorNames, varName, loc, compName, getOptValue=False):
        var = getattr(pyM, varName + '_' + self._abbrvName)
        factors = [getattr(self._componentsDict[compName], factorName)[loc] for factorName in factorNames]
        factor = 1.
        for factor_ in factors:
            factor *= factor_
        if not getOptValue:
            return (factor * sum(var[loc, compName, p, t] * esM._periodOccurrences[p]
                                 for p, t in pyM.timeSet)/esM._numberOfYears)
        else:
            return (factor * sum(var[loc, compName, p, t].value * esM._periodOccurrences[p]
                                 for p, t in pyM.timeSet)/esM._numberOfYears)

    def getLocEconomicsTI(self, pyM, factorNames, varName, loc, compName, divisorName='', getOptValue=False):
        var = getattr(pyM, varName + '_' + self._abbrvName)
        factors = [getattr(self._componentsDict[compName], factorName)[loc] for factorName in factorNames]
        divisor = getattr(self._componentsDict[compName], divisorName)[loc] if not divisorName == '' else 1
        factor = 1./divisor
        for factor_ in factors:
            factor *= factor_
        if not getOptValue:
    ...
    def getEconomicsTD(self, pyM, esM, factorNames, varName, dictName, getOptValue=False):
        indices = getattr(pyM, dictName + '_' + self._abbrvName).items()
        if self._dimension == '1dim':
    ...
    def setOptimalValues(self, esM, pyM, indexColumns, plantUnit, unitApp='', costApp=1):
        compDict, abbrvName = self._componentsDict, self._abbrvName
        capVar = getattr(esM._pyM, 'cap_' + abbrvName)
        binVar = getattr(esM._pyM, 'designBin_' + abbrvName)

        props = ['capacity', 'isBuilt', 'capexCap', 'capexIfBuilt', 'opexCap', 'opexIfBuilt', 'TAC',
                 'invest']
        units = ['[-]', '[-]', '[' + esM._costUnit + '/a]', '[' + esM._costUnit + '/a]', '[' + esM._costUnit + '/a]',
                 '[' + esM._costUnit + '/a]', '[' + esM._costUnit + '/a]', '[' + esM._costUnit + ']']
        ...
        optVal_ = utils.formatOptimizationOutput(values, 'designVariables', self._dimension, compDict=compDict)
        ...
        if optVal is not None:
            ...
            optSummary.loc[[(ix, 'invest', '[' + esM._costUnit + ']') for ix in i.index], i.columns] = \
                i.values * costApp
            optSummary.loc[[(ix, 'capexCap', '[' + esM._costUnit + '/a]') for ix in cx.index], cx.columns] = \
                cx.values * costApp
            optSummary.loc[[(ix, 'opexCap', '[' + esM._costUnit + '/a]') for ix in ox.index], ox.columns] = \
                ox.values * costApp
        ...
        if optVal is not None:
            ...
            optSummary.loc[[(ix, 'invest', '[' + esM._costUnit + ']') for ix in cx.index], cx.columns] += \
                i.values * costApp
