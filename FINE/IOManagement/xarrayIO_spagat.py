...
def convertEsmInstanceToXarrayDataset(esM, save=False, file_name="esM_instance.nc4"):
    ...
    xr_ds = utilsIO.addSeriesVariablesToXarray(
        xr_ds, component_dict, series_iteration_dict, locations
    )

    # STEP 6. Add all constant value variables to xr_ds
    xr_ds = utilsIO.addConstantsToXarray(
        xr_ds, component_dict, constants_iteration_dict
    )
...
def convertXarrayDatasetToEsmInstance(xarray_dataset):
    ...
    if isinstance(xarray_dataset, str):
        ...
        raise TypeError(
            "xarray_dataset must either be a path to a netcdf file or xarray dataset"
        )
