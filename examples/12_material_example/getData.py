...
def getData(engine="openpyxl"):
    ...
    # Onshore data
    capacityMax_full = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Wind", "maxCapacityOnshore_GW_el.xlsx"),
        index_col=0,
        engine=engine,
    ).squeeze("columns")

    print(capacityMax_full.index)
    print(capacityMax_full)

    capacityMax = capacityMax_full.loc[["GermanyRegion", "FranceRegion"]].copy()

    operationRateMax_full = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Wind", "maxOperationRateOnshore_el.xlsx"),
        index_col=0,
        engine=engine,
    )

    operationRateMax = operationRateMax_full[["GermanyRegion", "FranceRegion"]].copy()

    data.update({
        "Wind (onshore), capacityMax": capacityMax,
        "Wind (onshore), operationRateMax": operationRateMax
    })

    # Offshore data
    capacityMax = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Wind", "maxCapacityOffshore_GW_el.xlsx"),
        index_col=0,
        engine=engine,
    ).squeeze("columns")
    ...
    capacityFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "HydroPower", "fixCapacityROR_GW_el.xlsx"),
        index_col=0,
        engine=engine,
    ).squeeze("columns")

    operationRateFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "HydroPower", "fixOperationRateROR.xlsx"),
        header=0,
        index_col=0,
        engine=engine,
    )
    ...
    # Biogas data
    operationRateMax = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Biogas", "biogasPotential_GWh_biogas.xlsx"),
        header=0,
        index_col=0,
        engine=engine,
    )
    ...
    biogasCommodityCostTimeSeries = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Biogas", "biogasPriceTimeSeries_MrdEuro_GWh.xlsx"),
        header=0,
        index_col=0,
        engine=engine,
    )
    ...
    naturalGasCommodityCostTimeSeries = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "NaturalGas", "naturalGasPriceTimeSeries_MrdEuro_GWh.xlsx"),
        header=0,
        index_col=0,
        engine=engine,
    )
    ...
    capacityMax = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "NaturalGasPlants", "existingCombinedCycleGasTurbinePlantsCapacity_GW_el.xlsx"),
        index_col=0,
        engine=engine,
    ).squeeze("columns")
    ...
    capacityMax_full = (
        pd.read_excel(
            os.path.join(inputDataPath, "SpatialData", "GeologicalStorage", "existingSaltCavernsCapacity_GWh_methane.xlsx"),
            index_col=0,
            engine=engine,
        ).squeeze("columns") * 3 / 10
    )
    ...
    capacityFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "HydroPower", "fixCapacityPHS_storage_GWh_energyPHS.xlsx"),
        index_col=0,
        engine=engine,
    ).squeeze("columns")
    ...
    # AC cables
    capacityFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "ElectricGrid", "ACcableExistingCapacity_GW_el.xlsx"),
        index_col=0,
        header=0,
        engine=engine,
    )
    reactances = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "ElectricGrid", "ACcableReactance.xlsx"),
        index_col=0,
        header=0,
        engine=engine,
    )
    ...
    # DC cables
    capacityFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "ElectricGrid", "DCcableExistingCapacity_GW_el.xlsx"),
        index_col=0,
        header=0,
        engine=engine,
    )
    distances = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "ElectricGrid", "DCcableLength_km.xlsx"),
        index_col=0,
        header=0,
        engine=engine,
    )
    losses = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "ElectricGrid", "DCcableLosses.xlsx"),
        index_col=0,
        header=0,
        engine=engine,
    )
    ...
    eligibility = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Pipelines", "pipelineIncidence.xlsx"),
        index_col=0,
        header=0,
        engine=engine,
    )
    ...
    operationRateMax = operationRateMax_full[["GermanyRegion", "FranceRegion"]].copy()
    operationRateFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Demands", "electricityDemand_GWh_el.xlsx"),
        header=0,
        index_col=0,
        engine=engine,
    )
    operationRateFix = operationRateFix[["GermanyRegion", "FranceRegion"]].copy()

    data.update({"Electricity demand, operationRateFix": operationRateFix})

    # Hydrogen demand
    operationRateFix = pd.read_excel(
        os.path.join(inputDataPath, "SpatialData", "Demands", "hydrogenDemand_GWh_hydrogen.xlsx"),
        header=0,
        index_col=0,
        engine=engine,
    )
