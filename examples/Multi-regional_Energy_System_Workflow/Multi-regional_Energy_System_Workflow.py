...
rates = robustPipelineSizing.getInjectionWithdrawalRates(
    componentName="Pipelines (hydrogen)", esM=esM
)  # in GWh
...
op = (
    esM.componentModelingDict[esM.componentNames["Pipelines (hydrogen)"]]
    .getOptimalValues("operationVariablesOptimum")["values"]
    .loc["Pipelines (hydrogen)"]
)
rates = robustPipelineSizing.getInjectionWithdrawalRates(
    operationVariablesOptimumData=op
)  # in GWh
# Convert GWh to kg/s: GWh * (kWh/GWh) * (kg/kWh) * (1/ timestepLengthInSeconds) with timestepLengthInSeconds
# being 3600 seconds for the present example
rates = rates * (10 ** 6) * (1 / 33.32) * (1 / 3600)
...
(
    ...
    _,
) = robustPipelineSizing.determineDiscretePipelineDesign(
    robust=robust,
    injectionWithdrawalRates=rates,
    distances=lengths,
    dic_node_minPress=p_min_nodes,
    dic_node_maxPress=p_max_nodes,
    dic_diameter_costs=dic_diameter_costs,
    dic_candidateMergedDiam_costs=dic_candidateMergedDiam_costs,
    threads=7,
)
...
(
    ...
    gdfNodes,
    gdfEdges,
) = robustPipelineSizing.getRefinedShapeFile(
    transFilePath,
    regColumn1,
    regColumn2,
    dic_node_minPress,
...
(
    ...
    dic_node_minPress=dic_node_minPress_new,
    dic_node_maxPress=dic_node_maxPress_new,
    dic_diameter_costs=dic_diameter_costs,
    dic_candidateMergedDiam_costs=dic_candidateMergedDiam_costs,
    gdfEdges=gdfEdges,
    regColumn1="nodeIn",
...
fig, ax = robustPipelineSizing.plotOptimizedNetwork(
    gdfEdges, gdf_regions=gdf_regions, figsize=(5, 5), line_scaling=0.8
)
...
scen_min = dic_timeStep_PressLevels.loc[
    :, dic_timeStep_PressLevels.mean() == scen_min
].iloc[:, 0]
scen_max = dic_timeStep_PressLevels.mean().max()
scen_max = dic_timeStep_PressLevels.loc[
    :, dic_timeStep_PressLevels.mean() == scen_max
].iloc[:, 0]
