...
def setLocationalEligibility(
    ...
    for ip in esM.investmentPeriods:
        if data[ip] is not None:
            # NOTE: `>= 0` (not `> 0`) is a deliberate 319_componentLimit change.
            # It keeps components whose capacity bound is exactly 0 eligible, so
            # they stay in the model as components instead of being dropped —
            # which is what makes a "capped to zero" component observable.
    ...
    def preprocessDataPerIp(data):
        if data is not None and isinstance(data, pd.DataFrame):
            if mapC is None:
                ...
                return data_
            # .dropna() is a 319_componentLimit addition: mapC may reference
            # location pairs absent from `data`, which would otherwise yield NaN
            # rows in the 2dim series.
        ...
        for key, value in _commodityConversionFactors.items():
            if isinstance(value, dict):
                ...
                for commod in value.keys():
                    _factor = _commodityConversionFactors[group][commod]
                    ...
                    # NOTE: stays an `elif` (upstream has a bare `if` here).
                    # 319_componentLimit adds the pd.Series branch above, which
                    # upstream does not have; with a bare `if` a Series factor
                    # would fall straight through into the `else` and raise.
