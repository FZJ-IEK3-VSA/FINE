...
def _filterComponentParametersForInterval(
    ...
    for parameter_name, parameter_value in compEntry.items():
        ...
        elif parameter_name == "materialIntensity" and isinstance(parameter_value, dict):
            compEntry[parameter_name] = {
                year: value
                for year, value in parameter_value.items()
                if year in rollingHorizonYears or year in stockYears
            }

        elif parameter_name == "materialCollection" and isinstance(parameter_value, dict):
            compEntry[parameter_name] = {
                year: value
                for year, value in parameter_value.items()
                if year in rollingHorizonYears
            }
            continue
...
def _buildIntervalEsm(
    ...
    rollingHorizonEsmDict["materialDemandSameInvestmentPeriod"] = True
    rollingHorizonEsmDict["initialMaterialCost"] = {}
...
# =============================================================================
# Reoptimization helpers START
# =============================================================================

def getPathwayBalanceContributionForYears(
    esM,
    pathwayBalanceLimitID,
    years,
    loc="Total",
):
    """Return the signed pathway balance contribution for selected years.

    This uses the same component contribution logic as FINE's pathway balance
    constraint. For a CO2 sink, the returned value is usually negative because
    Sink components have negative sign in FINE.
    """
    if not hasattr(esM, "pyM") or esM.pyM is None:
        raise ValueError("The EnergySystemModel must be optimized first.")

    yearToIp = {
        year: ip
        for ip, year in enumerate(esM.investmentPeriodNames)
    }

    contribution = 0

    for mdlType, mdl in esM.componentModelingDict.items():
        if mdlType not in ("SourceSinkModel", "TransmissionModel"):
            continue

        componentNames = [
            compName
            for compName, comp in mdl.componentsDict.items()
            if comp.pathwayBalanceLimitID == pathwayBalanceLimitID
        ]
        ...
        for year in years:
            contribution += mdl.getBalanceLimitContribution(
                esM=esM,
                pyM=esM.pyM,
                ID=pathwayBalanceLimitID,
                ip=yearToIp[year],
    ...
    return pyomo.value(contribution) * esM.investmentPeriodInterval

def _isEmptyPathwayLimitValue(value):
    """Return True for empty pathway limit cells."""
    return value is None or pd.isna(value)

def getRemainingPathwayBalanceLimit(
    esM,
    solvedEsm,
    committedYears,
    pathwayBalanceLimit=None,
):
    """Return pathwayBalanceLimit reduced by committed pathway contributions.

    The returned DataFrame can be passed to a tail EnergySystemModel. It uses
    FINE's signed pathway contribution convention. For example, CO2 sinks
    contribute negatively, so subtracting committed negative emissions makes the
    tail CO2 budget less negative, i.e. smaller in absolute remaining emissions.
    """
    if pathwayBalanceLimit is None:
        pathwayBalanceLimit = esM.processedPathwayBalanceLimit

    if pathwayBalanceLimit is None:
        return None
    
    if not hasattr(solvedEsm, "pyM") or solvedEsm.pyM is None:
        raise ValueError(
            "solvedEsm must be optimized before remaining pathway limits "
            "can be computed."
        )

    remainingPathwayBalanceLimit = pathwayBalanceLimit.copy(deep=True)

    for pathwayBalanceLimitID, data in pathwayBalanceLimit.iterrows():
        for loc in list(esM.locations) + ["Total"]:
            value = data[loc]

            if _isEmptyPathwayLimitValue(value):
                continue

            committedContribution = getPathwayBalanceContributionForYears(
                esM=solvedEsm,
                pathwayBalanceLimitID=pathwayBalanceLimitID,
                years=committedYears,
                loc=loc,
            )

            remainingPathwayBalanceLimit.loc[
                pathwayBalanceLimitID,
                loc,
            ] = value - committedContribution

    return remainingPathwayBalanceLimit

def _getCommissioningFromSolvedEsm(solvedEsm, comp, year):
    """Read optimized commissioning of one component in one investment year."""
    if comp not in solvedEsm.componentNames:
        return None

    mdlClass = solvedEsm.componentNames[comp]

    try:
        commissioning = (
            solvedEsm
            .getOptimizationSummary(mdlClass, ip=year)
            .loc[comp, "commissioning"]
        )
    except (KeyError, AttributeError):
        return None

    if commissioning is None or not hasattr(commissioning, "loc"):
        return None

    commissioningByLocation = commissioning.loc[
        commissioning.index[0]
    ].T

    if round(commissioningByLocation.sum(), 5) <= 0:
        return None
...
def _getMaxLifetime(technicalLifetime):
    ...
    if isinstance(technicalLifetime, dict):
        values = []
        for value in technicalLifetime.values():
            values.append(value.max() if hasattr(value, "max") else value)
        return max(values)

    return technicalLifetime.max() if hasattr(technicalLifetime, "max") else technicalLifetime

def _pruneStockToLifetime(compEntry, firstYear):
    """Remove stock years that are older than the component lifetime."""
    stockData = compEntry.get("stockCommissioning")
    technicalLifetime = compEntry.get("technicalLifetime")

    if stockData is None or technicalLifetime is None:
        return

    maxLifetime = _getMaxLifetime(technicalLifetime)

    outdatedStockYears = [
        year
        for year in stockData
        if year < firstYear - maxLifetime
    ]

    for year in outdatedStockYears:
        stockData.pop(year)
...
def _addCommittedCommissioningToStock(
    ...
    """Add optimized baseline commissioning before the event as fixed stock."""
    compEntry["stockCommissioning"] = copy.deepcopy(
        compEntry.get("stockCommissioning")
    )

    for year in committedYears:
        commissioning = _getCommissioningFromSolvedEsm(
            solvedEsm,
            comp,
            year,
        )
        ...
        compEntry["stockCommissioning"][year] = commissioning

    _pruneStockToLifetime(compEntry, firstReoptimizationYear)

def _filterComponentParametersForReoptimization(
    compEntry,
    reoptimizationYears,
    stockYears,
    esM,
    ...
    for parameterName, parameterValue in compEntry.items():
        ...
        if not isinstance(parameterValue, dict) or not parameterValue:
            continue

        firstKey = list(parameterValue.keys())[0]

        if parameterName == "commodityConversionFactors":
            if firstKey in esM.investmentPeriodNames:
                compEntry[parameterName] = {
                    year: value
                    for year, value in parameterValue.items()
                    if year in reoptimizationYears
                }

            elif isinstance(firstKey, tuple):
                compEntry[parameterName] = {
                    (commissioningYear, operationYear): value
                    for (commissioningYear, operationYear), value
                    in parameterValue.items()
                    if operationYear in reoptimizationYears
                    and (
                        commissioningYear in reoptimizationYears
                        or commissioningYear in stockYears
                    )
                }

            continue

        if firstKey not in esM.investmentPeriodNames:
            continue
        ...
        if parameterName == "materialIntensity":
            ...
            relevantYears = reoptimizationYears
        ...
        compEntry[parameterName] = {
            year: value
            for year, value in parameterValue.items()
            if year in relevantYears
        }

def _buildReoptimizationComponentDict(
    compDict,
    solvedEsm,
    reoptimizationYears,
    committedYears,
    ...
    """Build component dict for the reoptimization tail model."""
    reoptimizationCompDict = copy.deepcopy(dict(compDict))

    for classname in reoptimizationCompDict:
        for comp in reoptimizationCompDict[classname]:
            compEntry = reoptimizationCompDict[classname][comp]
            ...
            _filterComponentParametersForReoptimization(
                compEntry,
                reoptimizationYears,
                stockYears,
                esM,
    ...
    return reoptimizationCompDict

def _buildReoptimizationEsm(
    esmDict,
    reoptimizationCompDict,
    reoptimizationYears,
    esM,
    pathwayBalanceLimit=None,
):
    """Construct the EnergySystemModel for the reoptimization tail."""
    reoptimizationEsmDict = esmDict.copy()
    reoptimizationEsmDict["startYear"] = reoptimizationYears[0]
    reoptimizationEsmDict["numberOfInvestmentPeriods"] = len(reoptimizationYears)

    for param, value in reoptimizationEsmDict.items():
        if isinstance(value, dict) and list(value.keys()) == esM.investmentPeriodNames:
            reoptimizationEsmDict[param] = {
                year: val
                for year, val in value.items()
                if year in reoptimizationYears
            }

    if pathwayBalanceLimit is not None:
        reoptimizationEsmDict["pathwayBalanceLimit"] = pathwayBalanceLimit

    reoptimizationEsmDict["materialDemandSameInvestmentPeriod"] = True
    reoptimizationEsmDict["initialMaterialCost"] = {}

    reoptimizationEsm = fn.EnergySystemModel(**reoptimizationEsmDict)

    for classname in reoptimizationCompDict:
        for comp in reoptimizationCompDict[classname]:
            reoptimizationEsm.add(
                getattr(fn, classname)(
                    esM=reoptimizationEsm,
                    **reoptimizationCompDict[classname][comp],
                )
            )

    return reoptimizationEsm

def buildReoptimizationEsm(
    esM,
    solvedEsm,
    reoptimizationStartYear,
    pathwayBalanceLimit=None,
):
    """Build a tail esM for event-triggered reoptimization.

    Parameters
    ----------
    esM:
        Full input EnergySystemModel with the updated assumptions, e.g.
        disrupted material import time series.
    solvedEsm:
        Already optimized baseline EnergySystemModel. All investment periods
        before reoptimizationStartYear are treated as committed decisions.
    reoptimizationStartYear:
        First investment period of the tail optimization.
    pathwayBalanceLimit:
        Optional explicit pathwayBalanceLimit for the tail. If None, the tail
        limit is computed automatically by subtracting committed pathway
        contributions from esM.processedPathwayBalanceLimit. If the original
        esM has no pathwayBalanceLimit, no pathwayBalanceLimit is passed to the
        tail model.
    """
    if reoptimizationStartYear not in esM.investmentPeriodNames:
        raise ValueError(
            f"reoptimizationStartYear {reoptimizationStartYear} is not part "
            f"of esM.investmentPeriodNames."
        )

    if reoptimizationStartYear not in solvedEsm.investmentPeriodNames:
        raise ValueError(
            f"reoptimizationStartYear {reoptimizationStartYear} is not part "
            f"of solvedEsm.investmentPeriodNames."
        )

    if esM.investmentPeriodInterval != solvedEsm.investmentPeriodInterval:
        raise ValueError(
            "esM and solvedEsm must use the same investmentPeriodInterval."
        )

    esmDict, compDict = fn.dictIO.exportToDict(esM)

    if "MaterialStorage" in compDict:
        raise NotImplementedError(
            "Reoptimization does not yet support MaterialStorage."
        )

    for comp in compDict.get("Storage", {}).values():
        if comp.get("stateOfChargeBoundary") == "interInvestmentPeriodNotCyclic":
            raise NotImplementedError(
                "Reoptimization does not yet support "
                "stateOfChargeBoundary='interInvestmentPeriodNotCyclic'."
            )

    committedYears = [
        year
        for year in solvedEsm.investmentPeriodNames
        if year < reoptimizationStartYear
    ]
    ...
    if pathwayBalanceLimit is None:
        pathwayBalanceLimit = getRemainingPathwayBalanceLimit(
            esM=esM,
            solvedEsm=solvedEsm,
            committedYears=committedYears,
        )

    reoptimizationCompDict = _buildReoptimizationComponentDict(
        compDict,
        solvedEsm,
        reoptimizationYears,
        committedYears,
    ...
    return _buildReoptimizationEsm(
        esmDict,
        reoptimizationCompDict,
        reoptimizationYears,
        esM,
...
def rollingHorizonOptimization(
    ...
    if "MaterialStorage" in compDict:
        raise NotImplementedError(
            "Rolling horizon does not yet support MaterialStorage. "
            "Disable MaterialStorage or implement SOC carry-over first."
        )

    for comp in compDict.get("Storage", {}).values():
        if comp.get("stateOfChargeBoundary") == "interInvestmentPeriodNotCyclic":
            raise NotImplementedError(
                "Rolling horizon does not yet support "
                "stateOfChargeBoundary='interInvestmentPeriodNotCyclic'."
            )
            
    def hasMaterialParameter(value):
        if value is None:
            return False
    ...
    hasMaterialParameters = any(
        hasMaterialParameter(comp.get("materialIntensity"))
        or hasMaterialParameter(comp.get("materialCollection"))
        for comps in compDict.values()
        for comp in comps.values()
    )

    if hasMaterialParameters:
        warnings.warn(
            "Rolling horizon uses same-investment-period material balancing. "
            "Material demand is balanced in the same investment period instead "
            "of the previous one."
        )

    if esmDict.get("initialMaterialCost"):
        warnings.warn(
            "initialMaterialCost is ignored in rolling horizon because material "
            "demand is balanced within the same investment period."
        )
