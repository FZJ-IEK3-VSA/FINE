...
def identifySolutions(
    ...
    # # ################################################################################################################
    """
    MGA optimization provides 2*(self.iterations) times different solutions. From these solutions, solutions 
...
def mgaOptimize(
    ...
    **Default arguments:**
    ...
    if solutions[0] is None:
        ...
        iteration (self.iterations). Each iteration has a minimization and a maximization of the optimization problem.
        therefore, each iteration provides 2 solutions (if the solutions are feasible). After the last iteration, 
        there will be a maximum of 2*(self.iterations) times final solutions. The optimization problem is defined in 
